...
4. [시스템 아키텍처](#시스템-아키텍처)
5. [통신 및 제어 흐름](#통신-및-제어-흐름)
6. [일정 계획 (Gantt 차트)](#일정-계획-gantt-차트)
7. [단계별 개발 계획](#단계별-개발-계획)
8. [시뮬레이션 완료 목표](#시뮬레이션-완료-목표)
9. [향후 계획](#향후-계획)
10. [사용 툴 및 패키지](#사용-툴-및-패키지)
11. [기존 논문 대비 개선점](#기존-논문-대비-개선점)
12. [참고문헌 & 논문](#참고문헌--논문)
...
이 프로젝트는 VSCode에서 바로 사용할 수 있도록 설정되어 있습니다.
...
2. **권장 확장 프로그램 설치**
   - VSCode에서 자동으로 권장 확장 프로그램 설치 알림 표시
   - Python, ROS2, C++ 개발 도구 자동 설치

3. **작업 실행**
   - `Ctrl+Shift+P` → "Tasks: Run Task" 선택
   - TurtleBot3 시뮬레이션, YOLO 학습/탐지 등 실행 가능

### 📁 VSCode 설정 파일

`.vscode/` 디렉토리에 다음 파일들이 포함되어 있습니다:
- `settings.json` - Python, ROS2, C++ 환경 설정
- `extensions.json` - 권장 확장 프로그램 목록
- `launch.json` - 디버깅 설정 (YOLO, Python, ROS2)
- `tasks.json` - 자동화 작업 (Gazebo, Nav2, YOLO 등)
- `c_cpp_properties.json` - C++ IntelliSense 설정
- `README.md` - 자세한 사용 가이드
