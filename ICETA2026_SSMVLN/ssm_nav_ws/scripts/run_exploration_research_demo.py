...
def write_summary(path, mission, aggs, viewer_path):
    lines = [
        ...
        "- ssm_utility mode uses a one-time-trained SSM-style policy to score frontier candidates, then falls back to frontier utility when unsafe",
...
def run_demo(args):
    ...
    model = None
    if any(mode in ("ssm", "ssm_utility") for mode in modes):
...
def main():
    ...
    p.add_argument("--modes", nargs="+", default=["utility", "ssm_utility"], choices=["nearest", "utility", "ssm", "ssm_utility"])
