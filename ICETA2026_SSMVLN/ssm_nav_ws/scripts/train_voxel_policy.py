...
class VoxelPolicyDataset(Dataset):
    def __init__(self, path):
        ...
        with open(path) as f:
            ...
            for row in reader:
                feat = [float(row[f"patch_{i}"]) for i in range(PATCH_SIZE)]
                feat += [
                    float(row["robot_z"]),
                    float(row["robot_r"]),
                    float(row["robot_c"]),
                ...
                self.y_reward.append([float(row[f"reward_{i}"]) for i in range(ACTIONS)])
...
class VoxelPolicyNet(nn.Module):
    def __init__(self, input_dim, hidden_dim=128, layers=3, actions=ACTIONS):
        super().__init__()
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for x, y_action, y_reward in loader:
            ...
            loss = F.cross_entropy(logits, y_action) + 0.2 * F.mse_loss(reward_pred, y_reward)
...
def train(args):
    base_train = VoxelPolicyDataset(args.train)
    train_ds = fractional_subset(base_train, args.train_fraction, args.seed)
    val_ds = VoxelPolicyDataset(args.val)
    input_dim = base_train.x.shape[1]
    device = torch.device("cuda" if torch.cuda.is_available() and not args.cpu else "cpu")

    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False)
    model = VoxelPolicyNet(input_dim, args.hidden_dim, args.layers).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)

    best_acc = -1.0
    Path(args.log).parent.mkdir(parents=True, exist_ok=True)
    with open(args.log, "w") as f:
        f.write("epoch,train_fraction,train_loss,train_acc,val_loss,val_acc,best_acc\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with open(args.log, "a") as f:
            f.write(f"{epoch},{args.train_fraction},{train_loss},{train_acc},{val_loss},{val_acc},{best_acc}\n")
