...
<body>
  <div class="shell">
    <aside>
      ...
      <div class="section">
        ...
        <div class="metric-grid">
          <div class="metric"><strong id="mSuccess">-</strong><span>SSM success</span></div>
          <div class="metric"><strong id="mSteps">-</strong><span>SSM avg target step</span></div>
    ...
    <main>
      ...
      <div class="bottom">
        <div class="panel">
          <h2>Paper-Proxy Comparison</h2>
          <p class="panel-note">MTU3D-style Proxy is a local simulator approximation, not the official MTU3D benchmark.</p>
  ...
  <script>
    ...
    function modeLabel(value) {
      if (value === 'ssm_utility') return 'Proposed SSM';
      if (value === 'hybrid_ssm') return 'Hybrid SSM';
      if (value === 'mtu3d_proxy') return 'MTU3D-style Proxy';
      if (value === 'utility') return 'Utility Control';
      if (value === 'nearest') return 'Nearest Control';
      if (value === 'ssm') return 'SSM Only';
      return value;
    }
    ...
    function setupControls() {
      ...
      modeBox.innerHTML = DATA.modes.map(m => `<button data-mode="${m}">${modeLabel(m)}</button>`).join('');
    ...
    async function runNaturalLanguageCommand() {
      ...
      if (window.location.protocol === 'file:') {
        status.textContent = '현재 파일로 열려 있어 Python 재실행이 막혀 있습니다. 터미널에서 cd /home/mj/my_research/ssm_nav_ws 후 python3 scripts/voxel_study_dashboard_server.py 를 실행하고 http://127.0.0.1:8787 로 접속하세요.';
        return;
      }
      button.disabled = true;
      button.textContent = 'Running...';
      status.textContent = '명령을 grounding하고 unseen voxel map 평가를 다시 실행 중입니다.';
      try {
    ...
    function updateBars() {
      ...
      document.getElementById('bars').innerHTML = DATA.aggregate.map(row => {
        ...
        const color = row.mode === 'ssm_utility' ? 'var(--teal)' : (row.mode === 'mtu3d_proxy' ? 'var(--blue)' : 'var(--amber)');
        return `
          <div class="bar-row"><span>${modeLabel(row.mode)}</span><div class="track"><div class="fill" style="width:${width}%;background:${color}"></div></div><span>${fmt(steps, 2)}</span></div>
    ...
    function refreshCase(resetStep) {
      ...
      document.getElementById('caseTitle').textContent = `${mapName} / ${modeLabel(mode)}`;
    ...
    function drawCommandTargetMarker(cd) {
      if (!controls.target.checked || !cd.target || !targetVisibleAtStep(cd)) return;
    ...
    function observedByStep(v) {
      if (v.value === -1) return controls.unknown.checked;
      const firstSeen = Number(v.first_seen_step ?? -1);
      if (!Number.isFinite(firstSeen) || firstSeen < 0) {
        return !('first_seen_step' in v);
      }
      return firstSeen <= step;
    }

    function targetVisibleAtStep(cd) {
      const targetSeenStep = Number(cd.metrics.target_seen_step ?? -1);
      return targetSeenStep >= 0 && step >= targetSeenStep;
    }

    function currentObservedNodeCount(cd) {
      return cd.observed.filter(v => v.value !== -1 && v.value !== 1 && observedByStep(v)).length;
    }
    ...
    function draw() {
      ...
      document.getElementById('kNodes').textContent = currentObservedNodeCount(cd);
      drawAxes(cd);
      const faces = [];
      ...
      if (controls.observed.checked) {
        for (const v of cd.observed) if (v.value === 0 && observedByStep(v)) faces.push(...cubeFaces(v, cd, '#f7f5ee', .43, .74));
      }
      if (controls.obstacles.checked) {
        for (const v of cd.mapVoxels) if (v.value === 1) faces.push(...cubeFaces(v, cd, '#1f2429', .56, .82));
      }
      if (controls.target.checked && targetVisibleAtStep(cd)) {
    ...
    return [
        {
            ...
            "value": int(r["value"]),
            "visits": int(r["visits"]),
            "first_seen_step": int(r.get("first_seen_step", -1)),
        }
        for r in read_csv(path)
    ]
